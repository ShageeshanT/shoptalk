...
def build_health_check() -> HealthCheck:
    checks = [
        ...
        HealthCheckItem(name="approvals", ok=len(state.approvals.list()) >= 0, detail="Approval queue is reachable."),
        HealthCheckItem(name="follow_ups", ok=len(state.follow_ups.list()) >= 0, detail="Follow-up store is reachable."),
    ]
