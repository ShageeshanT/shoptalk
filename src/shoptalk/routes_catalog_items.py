...
from shoptalk.catalog_store import create_catalog_item, list_catalog_items
...
def list_items(business_id: UUID, available_only: bool = True) -> list[CatalogItemRecord]:
    if state.businesses.get(business_id) is None:
        raise HTTPException(status_code=404, detail="Business not found")
    return list_catalog_items(business_id, available_only=available_only)
