...
def draft_checkout(order_id: UUID) -> CheckoutDraft:
    ...
    message = (
        ...
        f"Please pay {amount_text} and send the receipt here so we can confirm it."
    )
    return CheckoutDraft(order_id=order.id, amount=amount, currency=currency, message=message)
